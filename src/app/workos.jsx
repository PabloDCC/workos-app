...
export default function WorkOS() {
  ...
  const [settings, setSettings] = useState({ areas: DEFAULT_AREAS, priorities: DEFAULT_PRIORITIES, states: DEFAULT_STATES, team: DEFAULT_TEAM, darkMode: false });
  const [loaded, setLoaded] = useState(false);
  const [saving, setSaving] = useState(false);
  const [selectedTask, setSelectedTask] = useState(null);
  const [showTaskForm, setShowTaskForm] = useState(false);
  const [filters, setFilters] = useState({ area:"", priority:"", state:"", assignee:"", search:"" });
  const [newTask, setNewTask] = useState({ title:"", area:"", priority:"Media", state:"Pendiente", assignee:"Yo", deadline:"", description:"" });
  ...
  useEffect(() => {
    (async () => {
      const [t, s] = await Promise.all([store.get("wos_tasks", SAMPLE_TASKS), store.get("wos_settings", { areas:DEFAULT_AREAS, priorities:DEFAULT_PRIORITIES, states:DEFAULT_STATES, team:DEFAULT_TEAM, darkMode:false })]);
  ...
  const addTask = () => {
    ...
    setNewTask({ title:"", area:"", priority:"Media", state:"Pendiente", assignee:"Yo", deadline:"", description:"" });
  ...
  const clearFilter = () => setFilters({ area:"", priority:"", state:"", assignee:"", search:"" });
        ...
        {view==="settings" && (
          <div style={{ maxWidth:600, margin:"0 auto" }}>
            ...
            <div style={{ display:"flex", gap:6, marginBottom:16, overflowX:"auto" }}>
              {[["areas","Áreas"],["priorities","Prioridades"],["states","Estados"],["team","Equipo"],["app","App"]].map(([k,l])=>(
