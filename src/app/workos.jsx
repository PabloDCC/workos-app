...
const PRIORITY_COLORS = {
  "Urgente":"#dc2626","Alta":"#ea580c","Media":"#ca8a04","Baja":"#6b7280",
};
...
const SAMPLE_TASKS = [
  { id:1, title:"Tramitar licencia de construcción Proyecto Arboleda", area:"Permisos", priority:"Urgente", state:"En progreso", assignee:"Yo", deadline:"2026-06-15", project:"Aguamarina", description:"Gestión ante el municipio para obtener la licencia de construcción.", notes:[{id:1,date:"2026-05-10",author:"Yo",text:"Entregada solicitud al municipio con planos firmados."},{id:2,date:"2026-05-22",author:"Yo",text:"En revisión técnica. Esperamos respuesta en 2 semanas."}], created:"2026-05-01" },
  { id:2, title:"Revisión contrato de obra Torre Sur", area:"Legal y Notaría", priority:"Alta", state:"Pendiente", assignee:"Yo", deadline:"2026-06-01", project:"Turquesa", description:"Revisión y negociación del contrato de obra civil con el contratista principal.", notes:[], created:"2026-05-15" },
  { id:3, title:"Campaña de lanzamiento Residencial Cumbres", area:"Marketing", priority:"Media", state:"En progreso", assignee:"Sin asignar", deadline:"2026-06-10", project:"Jade", description:"Coordinación de campaña digital para lanzamiento del proyecto.", notes:[{id:1,date:"2026-05-15",author:"Yo",text:"Brief enviado a agencia. Esperamos propuesta esta semana."}], created:"2026-05-10" },
];
...
export default function WorkOS() {
  ...
  useEffect(() => {
    (async () => {
      const [t, s] = await Promise.all([
        store.get("wos_tasks", SAMPLE_TASKS),
        store.get("wos_settings", { areas:DEFAULT_AREAS, priorities:DEFAULT_PRIORITIES, states:DEFAULT_STATES, team:DEFAULT_TEAM, projects:DEFAULT_PROJECTS, darkMode:false })
      ]);
  ...
  // Check Google OAuth token on load
  useEffect(() => {
    ...
    if (token) {
      ...
      if (refresh) localStorage.setItem("gcal_refresh", refresh);
  ...
  const startVoice = () => {
    ...
    rec.onresult = e => { const t = Array.from(e.results).map(r=>r[0].transcript).join(""); setVoiceTranscript(t); recognitionRef._last = t; };
    rec.onend = () => { setListening(false); const txt = recognitionRef._last; if(txt?.trim()) { setChatInput(txt); handleVoiceSubmit(txt); recognitionRef._last = ""; } };
    rec.onerror = () => setListening(false);
    recognitionRef.current = rec;
    rec.start();
  };

  const handleVoiceSubmit = async (text) => { if (!text.trim()) return; setView("chat"); await sendAIMessage(text); };
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          if (action.action==="create_task" && action.data) {
            const t = { id:uid(), notes:[], created:nowStr(), state:"Pendiente", priority:"Media", assignee:"Yo", area:"", deadline:"", description:"", project:"", ...action.data };
            ...
            setChatMsgs([...next, { role:"assistant", content:"✅ Nota agregada." }]);
  ...
  const disconnectGcal = () => {
    ...
    localStorage.removeItem("gcal_refresh");
  ...
  const createGcalEvent = async () => {
    ...
    try {
      ...
      const endDT = newEvent.time
        ? new Date(new Date(startDT).getTime() + newEvent.duration * 60000).toISOString()
        : newEvent.date;
      const event = {
      ...
      const res = await fetch("/api/calendar", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ token: gcalToken, event })
      });
      const data = await res.json();
      if (data.id) {
        ...
        setTimeout(loadGcalEvents, 1000);
      } else {
  ...
  const addNote = (taskId) => {
    ...
    const updated = tasks.map(t => t.id===taskId ? {...t, notes:[...(t.notes||[]), note]} : t);
    setTasks(updated);
  ...
  const S = {
    ...
    btn: (v) => ({
      background: v==="primary"?c.accent:v==="gold"?c.gold:v==="danger"?"#dc2626":v==="green"?"#16a34a":c.surface2,
      color: ["primary","gold","danger","green"].includes(v)?"#fff":c.text,
      border: "none", borderRadius:8, padding:"8px 16px", cursor:"pointer", fontSize:12,
      fontWeight:700, fontFamily:"inherit", transition:"all 0.15s", letterSpacing:0.3
    }),
  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.main}>
          ...
          <>
            ...
            <div style={S.card}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                {(settings.projects||DEFAULT_PROJECTS).map(proj=>{
                  ...
                  return (
                    <div key={proj} onClick={()=>{setFilters({...filters,project:proj});setView("tasks");}} style={{ background:"#e0f2fe", border:"1px solid #bae6fd", borderRadius:10, padding:"10px 14px", textAlign:"center", cursor:"pointer", minWidth:90 }}>
            ...
            <div style={S.card}>
              ...
              <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                ...
                <button style={S.btn("green")} onClick={startVoice}>🎙 Hablar</button>
                <button style={S.btn(null)} onClick={()=>setView("chat")}>🤖 IA</button>
              </div>
            </div>
          </>
        )}
          ...
          <>
            ...
            {showTaskForm && (
              <div style={{ ...S.card, border:"2px solid " + c.accent, marginBottom:14 }}>
                ...
                <div style={{ marginBottom:12 }}><label style={S.label}>Descripción</label><textarea style={{ ...S.input, resize:"vertical", minHeight:60 }} value={newTask.description} onChange={e=>setNewTask({...newTask,description:e.target.value})} placeholder="Detalles..." /></div>
          ...
          <>
            ...
            {!gcalConnected ? (
              <div style={{ ...S.card, textAlign:"center", padding:32 }}>
                ...
                <div style={{ color:c.text2, fontSize:13, marginBottom:20 }}>Vinculá tu cuenta para ver y crear eventos directamente desde WorkOS.</div>
              ...
              <>
                ...
                {!gcalLoading && gcalEvents.length===0 && (
                  <div style={{ ...S.card, textAlign:"center", color:c.text2, padding:24 }}>No hay eventos en los próximos 14 días</div>
                )}
                {gcalEvents.map((ev,i)=>(
            ...
            {showNewEvent && (
              <div style={S.modal} onClick={()=>setShowNewEvent(false)}>
                <div style={S.modalBox} onClick={e=>e.stopPropagation()}>
                  ...
                  <div style={{ marginBottom:14 }}><label style={S.label}>Descripción</label><input style={S.input} value={newEvent.desc} onChange={e=>setNewEvent({...newEvent,desc:e.target.value})} /></div>
                  <div style={{ display:"flex", gap:8 }}>
        ...
        {view==="chat" && (
          <div style={{ maxWidth:680, margin:"0 auto" }}>
            ...
            <div style={{ display:"flex", gap:8, marginTop:10 }}>
              <button onClick={startVoice} style={{ ...S.btn(listening?"danger":null), padding:"9px 14px", fontSize:18, flexShrink:0 }}>{listening?"⏹":"🎙"}</button>
            ...
            <div style={{ marginTop:12, display:"flex", flexWrap:"wrap", gap:6 }}>
              {["¿Qué documentos necesito para una licencia de construcción?","¿Cómo calculo el precio por m²?","Checklist entrega de obra","Cláusulas clave en contrato de compraventa"].map(q=>(
        ...
        {view==="settings" && (
          <div style={{ maxWidth:600, margin:"0 auto" }}>
            ...
            {settingsTab!=="app" && (
              <div style={S.card}>
                <div style={S.sTitle}>{settingsTab==="areas"?"Áreas":settingsTab==="priorities"?"Prioridades":settingsTab==="states"?"Estados":settingsTab==="projects"?"Proyectos":"Equipo"}</div>
            ...
            {settingsTab==="app" && (
              <div style={S.card}>
                ...
                <div style={{ padding:"12px 0", borderBottom:"1px solid " + c.border }}>
                  ...
                  <div style={{ fontSize:12, color:c.text2, marginBottom:8 }}>{gcalConnected?"✅ Conectado y activo":"No conectado"}</div>
      ...
      <button style={S.voiceBtn} onClick={startVoice}>
